...
/* grabbing a primary key for a record
 ...
 $stmt = $db->prepare($sql);
  ...
  $stmt->execute();
  $pk = $db->lastInsertId();   gets the key of the last inserted row zero if it didn't happen
*/
$pwd = "foo";
$hash= password_hash($pwd, PASSWORD_DEFAULT); // used for storing the pwd in db
$pwd = "foo";
echo "<p>" . $hash . "</p>";
$pwd = "foo";
